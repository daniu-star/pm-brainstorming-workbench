 ...
 * Hard caps are absolute. Defaults tuned for easier trigger: iris 0.12, yaw 18°, pitch 14°.
 *
 * `looking_away` is a sustained episode (≥ awayEnterMs), not a short edge.
 * `face_missing` / `multi_face` use presenceDebounceMs only (~500ms).
 */
export type GazeRulesConfig = {
  /**
   * Soft enter: |yaw − baseline.yaw| (deg). Default 18.
   */
  yawDegThreshold?: number;
  /** Soft enter: |pitch − baseline.pitch| (deg). Default 14. */
  pitchDegThreshold?: number;
  /**
   * Soft enter: ‖δgaze‖ vs baseline (dx÷halfW, dy÷halfH). Default 0.12.
   ...
   * Default 1500ms (sustained episode, not ~0.5s edge).
   */
  awayEnterMs?: number;
  /** Sustained OK required after away before emitting `face_ok`. Default 500ms. */
  okRecoverMs?: number;
  /**
