...
const EXPERTS = [
  { name: "CTO", caption: "技术与可行性", color: "text-sky-300", icon: CodeIcon },
  { name: "产品设计", caption: "体验与认知", color: "text-cyan-300", icon: PaletteIcon },
  { name: "商业运营", caption: "市场与增长", color: "text-emerald-300", icon: ChartIcon },
  { name: "目标用户", caption: "需求与采用", color: "text-amber-300", icon: UserIcon },
];
...
export default function LandingPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const session = await api<{ id: string }>("/api/session", {
        method: "POST",
        body: JSON.stringify({ problem_statement: problem.trim() }),
      });
      router.push(`/session/${session.id}?problem=${encodeURIComponent(problem.trim())}`);
    } catch (err) {
      setIsCreating(false);
      setError(err instanceof Error ? err.message : "创建会话失败，请确认后端服务已启动");
    }
  };
  ...
  return (
    <main id="main-content" className="landing-shell min-h-dvh overflow-hidden text-zinc-100">
      ...
      <header className="fixed inset-x-0 top-0 z-30 border-b border-white/10 bg-dark-900/90 px-5 safe-top">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/product"
              className="hidden h-10 items-center gap-2 rounded-lg border border-white/10 px-4 text-xs font-medium text-zinc-300 transition-colors duration-150 hover:border-cyan-300/30 hover:bg-cyan-300/5 hover:text-white sm:flex"
            >
              <PlayIcon size={14} />
              产品全景
            </Link>
      ...
      <section className="relative z-10 mx-auto grid min-h-dvh max-w-7xl items-center gap-14 px-5 pb-20 pt-28 lg:grid-cols-[1.08fr_0.92fr] lg:px-8">
        <div className="hero-reveal max-w-3xl">
          ...
          <p className="mb-4 text-xs font-semibold uppercase text-cyan-200/70">
            Product Decision Intelligence / 01
          </p>
          ...
          <div className="mt-9 flex flex-wrap items-center gap-3">
            <a
              href="#start"
              className="inline-flex h-12 items-center gap-2 rounded-lg bg-cyan-300 px-6 text-sm font-semibold text-[#031014] shadow-lg shadow-cyan-950/40 transition-transform duration-150 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-cyan-200 focus:ring-offset-2 focus:ring-offset-[#06090e]"
            >
              启动产品推演
            ...
            <Link
              href="/product"
              className="inline-flex h-12 items-center gap-2 rounded-lg border border-white/15 bg-white/[0.03] px-6 text-sm font-medium text-zinc-200 transition-colors duration-150 hover:border-white/30 hover:bg-white/[0.06]"
            >
              <PlayIcon size={16} />
              了解产品如何工作
            </Link>
        ...
        <div id="start" className="command-panel hero-reveal hero-reveal-delay scroll-mt-24">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-px border-b border-white/10 bg-white/10">
              {EXPERTS.map((expert) => {
                ...
                return (
                  <div key={expert.name} className="flex items-center gap-3 bg-dark-800 px-4 py-3.5">
            ...
            <div className="p-5">
              ...
              <textarea
                ...
                rows={5}
                className="w-full resize-none rounded-xl border border-white/10 bg-black/20 px-4 py-3.5 text-sm leading-6 text-zinc-100 outline-none transition-colors duration-150 placeholder:text-zinc-500 focus:border-cyan-300/50 focus:ring-2 focus:ring-cyan-300/10"
                onKeyDown={(event) => {
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {QUICK_STARTS.map((item) => (
                  <button
                    ...
                    className="rounded-md border border-white/10 px-2.5 py-1.5 text-left text-[10px] text-zinc-500 transition-colors duration-150 hover:border-cyan-300/30 hover:text-cyan-100"
                  >
                    {item}
                  </button>
                ))}
              ...
              <button
                ...
                className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-cyan-300 text-sm font-semibold text-[#031014] transition-colors duration-150 hover:bg-cyan-200 disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:text-zinc-500"
              >
                {isCreating ? (
                  <>
      ...
      <section className="relative z-10 border-y border-white/10 bg-dark-900/90 px-5 py-20 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-px overflow-hidden rounded-xl border border-white/10 bg-white/10 md:grid-cols-4">
            {WORKFLOW.map((item) => (
              <article key={item.index} className="group bg-dark-800 p-6 transition-colors duration-150 hover:bg-dark-700">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {[
              { icon: LayersIcon, title: "结构化，不止是聊天", body: "功能、风险、问题和洞察持续沉淀到同一张产品画布。" },
              { icon: ShieldIcon, title: "审计式压力测试", body: "六维问题框架主动寻找方案中的盲区、依赖与不可证伪假设。" },
              { icon: SearchIcon, title: "为决策保留证据", body: "每一个结论都能回到提出它的角色和上下文，减少拍脑袋决策。" },
            ].map((item) => {
      ...
      <footer className="relative z-10 px-5 py-8 text-center text-[11px] text-zinc-400">
